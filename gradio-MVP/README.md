...
The app loads menu files at runtime and supports a tabbed MVP flow: menu preview, cart builder, guest/member checkout with saved-order evidence, and an MVP admin snapshot. It covers pizza, base, one or more toppings, quantity per line, customer validation, delivery address validation, guest online-payment rules, checkout, GST, discount, payment confirmation, parseable order logging, and session-only menu customisation from the admin tab. If the grader swaps the three `.txt` files with new valid files using the same `ID;Name;Price` format, the UI will display the new menu without code changes.
